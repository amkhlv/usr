...
class Aux(object):
    ...
    def emacsify(self, tkinter_text):
        ...
        def altB(e):
            ...
            if (tkinter_text.index(Tkinter.INSERT) == tkinter_text.index(
                        tkinter_text.index(Tkinter.END) + " - 1 chars")):
        ...
        def altBS(e):
            ...
            if startpoint_ == "":
                ...
                startpoint_ = startpoint_ + " + 1 chars "
            tkinter_text.clipboard_clear()
        ...
        def controlG(e):
            try:
                ...
                tkinter_text.mark_unset(Tkinter.SEL_FIRST);
                tkinter_text.mark_unset(Tkinter.SEL_LAST)
    ...
    class _DialogOutline(object):
        ...
        def insert_textfields(self, prefill, locked=False):
            ...
            for itm in self.columnspecs:
                ...
                self.txtfield[itm].pack(side=Tkinter.RIGHT, fill=Tkinter.BOTH,
                                        expand=ao.cnf.expand_ver or ao.cnf.expand_hor)
    ...
    class _Buttons:
        def __init__(self, my, specs, criterium, show_cols,
            ...
            vardict = dict([(x, locals()[x]) for x in arglist[1:]])
            ...
            tblname = self.specs['tablename'];
            columnspecs = self.specs['columns'];
        ...
        def delwin(self):
            if self.mainwin_but:
                ...
                print("mainwin_but already destroyed")
...
def update(specs, criterium, data_tuple=None, buttonsObj=None, readonly=False):
    ...
    newdata_qmarks = ", ".join([z[0] + " = ?" for z in specs['columns']])
    where_qmarks   = " AND ".join(["eqli(" + z[0] + ",?)" for z in specs['columns']])
    ordered_cols   = ",".join([z[0] for z in specs['columns']])
    ...
    def cmd_fn(checked, specs, cmdline, widthline):
        def returned_fn(event):
            ...
            # print("NO CHECKED")
                ...
                show_cols = ",".join([x[0] for x in checked_columns])
            ...
            if hist_back and hist_back[-1] == cmdline_content:
                pass
            elif hist_forw and hist_forw[-1] == cmdline_content:
                pass
            else:
                ...
                if p.search(str(b.encode('ascii', 'ignore'))):
                    ...
                    for b1 in zip(c, list(a)):
                        if b1[1] or j == 1:
                            und = "-" if j == 1 else "-"
                            color = my.clr_show_sep1 if j == 1 else my.clr_show_sep2
                            print("   " + color + und * (4 - 2 * j) + my.clr_normal + " " + my.bold + b1[
                                0] + my.reset + ": " +
...
#    from IPython.Shell import IPShellEmbed
    ...
    parser.add_option("--setup", dest="do_setup", action="store_true", default=False,
