use cursive::views::{Button, Dialog, EditView, LinearLayout, PaddedView, Panel, TextView};
use cursive::CbSink;
...
fn patiently<T: Future>(f: T) -> Timeout<T> {
...
async fn main() -> Result<(), Error> {
    ...
    let prog = args.next().unwrap_or_else(|| "read-db-conf".to_string());
        ...
        loop {
            match rx_com.recv().await {
                Some(DBComm::ShowToDos) => {
                    ...
                    cb_sink
                        .send(Box::new(move |s| {
                            ...
                            vbox = vbox.child(EditView::new().on_submit(move |_win, x| {
                ...
                Some(DBComm::InsertToDo(x)) => {
                    ...
                    tx2.send(DBComm::ShowToDos)
                        .await
                        .expect("could not send SELECT when inserting");
                }
                Some(DBComm::NewList(name)) => {
                    wait(&cb_sink, "INSERT-ing");
                    let tx2 = tx1.clone();
                    patiently(client.execute(
                        "insert into lists (title, items) values ($1, '{}'::text[])",
                        &[&name],
                    ))
                    .await
                    .check_timeout(&cb_sink)
                    .expect("failed to create new list");
                    tx2.send(DBComm::ShowList(name))
                        .await
                        .expect("could not send SELECT when inserting");
                }
                Some(DBComm::InsertItemIntoList(itm, lst)) => {
                    ...
                    .await
                    .check_timeout(&cb_sink)
                    .expect("failed to add an item");
                    let tx2 = tx1.clone();
                    tx2.send(DBComm::ShowList(lst))
                        .await
                        .expect("could not send SELECT when inserting");
                }
                Some(DBComm::DeleteItemFromList(itm, lst)) => {
                    ...
                    tx2.send(DBComm::ShowList(lst))
                        .await
                        .expect("could not send ShowList when deleting");
                }
                Some(DBComm::ConfirmDeleteToDo(x)) => {
                    ...
                    cb_sink
                        .send(Box::new(move |s| {
                            ...
                            s.pop_layer();
                            s.add_layer(
                                Dialog::text(format!("Really delete {} ?", x))
                                    .title("Confirmation")
                ...
                Some(DBComm::DeleteList(name)) => {
                    wait(&cb_sink, "DELETE-ing");
                    patiently(client.execute("delete from lists where title = $1", &[&name]))
                        .await
                        .check_timeout(&cb_sink)
                        .expect("failed to delete");
                    tx1.send(DBComm::ShowLists)
                        .await
                        .expect("error sending ShowLists");
                }
                Some(DBComm::ConfirmDeleteList(name)) => {
                    ...
                    cb_sink
                        .send(Box::new(move |s| {
                            ...
                            s.pop_layer();
                            s.add_layer(
                                Dialog::text(format!("Delete WHOLE LIST {} ?", x1))
                                    .title("Confirmation")
                                    .button("Yes!", move |_win| {
                ...
                Some(DBComm::ConfirmDeleteItemFromList(itm, lst)) => {
                    ...
                    cb_sink
                        .send(Box::new(move |s| {
                            ...
                            s.pop_layer();
                            s.add_layer(
                                Dialog::text(format!("Delete {} from {} ?", itm, lst))
                                    .title("Confirmation")
                                    .button("Yes!", move |_win| {
                                        block_on(txyes.send(DBComm::DeleteItemFromList(
                                            x1.clone(),
                                            y1.clone(),
                                        )))
                ...
                Some(DBComm::ShowLists) => {
                    ...
                    cb_sink
                        .send(Box::new(move |s| {
                            let mut vbox = LinearLayout::vertical();
                            vbox = vbox.child(EditView::new().on_submit(move |_win, t| {
                            ...
                            for row in rows {
                                ...
                                vbox = vbox.child(PaddedView::lrtb(
                                    0,
                                    0,
                                    1,
                                    0,
                                    Button::new(ttl.clone(), move |_win| {
                ...
                Some(DBComm::ShowList(listname)) => {
                    wait(&cb_sink, "SELECT-ing");
                    let lsts = patiently(
                        client.query("select * from lists where title = $1", &[&listname]),
                    )
                    .await
                    .check_timeout(&cb_sink)
                    .expect("could not SELECT");
                    ...
                    cb_sink
                        .send(Box::new(move |s| {
                            ...
                            vbox =
                                vbox.child(EditView::new().on_submit(move |_win, itm| {
                                    block_on(tx2.send(DBComm::InsertItemIntoList(
                                        itm.to_owned(),
                                        ln1.clone(),
                                    )))
                            ...
                            if let Some(lst) = lsts.iter().next() {
                                ...
                                for item in items {
                                    ...
                                    vbox = vbox.child(Button::new(item.clone(), move |_win| {
                                        block_on(txdel.send(DBComm::ConfirmDeleteItemFromList(
                                            x.clone(),
                                            title2.clone(),
                                        )))
                            ...
                            vbox =
                                vbox.child(Panel::new(Button::new("DELETE LIST", move |_win| {
                                    block_on(
                                        txdelist.send(DBComm::ConfirmDeleteList(which.clone())),
                                    )
                                    .expect("error sending ConfirmDeleteList")
                                })));
                            let txback = tx3.clone();
                            vbox =
                                vbox.child(Panel::new(Button::new("BACK to lists", move |_win| {
    ...
    siv.add_layer(
        Dialog::text(format!("welcome"))
            ...
            .button("ToDo", move |_win| {
                block_on(tx1.send(DBComm::ShowToDos)).expect("could not send DeleteToDo");
            })
            .button("Lists", move |_win| {
                block_on(tx2.send(DBComm::ShowLists)).expect("could not send ShowToDos");
            }),
    );
    siv.run();
    println!("exiting");
